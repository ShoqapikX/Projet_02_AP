...
public class PanelClientPart extends PanelPrincipal implements ActionListener{
	...
	public PanelClientPart() {
		...
		//Chaque tour de boucle pose les éléments sur une ligne
		for (int i = 0; i < textfields.length; i++) {
			//Mise en place textfield
	...
	public void actionChamps(int choix) {		
		switch (choix) {
		case 1: 
			//On appelle la méthode qui va traiter les données à inserer
			this.traitement();
			break;
		case 2:
			//Boucle pour vider tous les jtextfield
			for (int i = 0; i < textfields.length; i++) {
				textfields[i].setText("");
			}
			//Vider l'emplacement mdp
			this.txtMdp.setText("");
			break;
		}
	}
